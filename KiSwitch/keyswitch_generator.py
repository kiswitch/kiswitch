...
path3d = "${KICAD6_3RD_PARTY}/3dmodels/" "com_github_perigoso_keyswitch-kicad-library/" "3d-library.3dshapes/"
...
def generate_switch_alps_matias(output_path):
    ...
    for key in keys:
        switches.append(SwitchAlpsMatias(path3d=path3d, keycap=Keycap(spacing=spacing, **keycaps[key])))
...
def generate_switch_cherry_mx(output_path):
    ...
    for switch_type in ["PCB", "Plate"]:
        ...
        for key in keys:
            switches.append(
                SwitchCherryMX(switch_type=switch_type, path3d=path3d, keycap=Keycap(spacing=spacing, **keycaps[key]))
            )
...
def generate_switch_hybrid_cherry_mx_alps(output_path):
    ...
    for key in keys:
        switches.append(SwitchHybridCherryMxAlps(path3d=path3d, keycap=Keycap(spacing=spacing, **keycaps[key])))
...
def generate_switch_kailh_choc(output_path):
    ...
    for switch_type in ["V1", "V2", "V1V2"]:
        switches.append(SwitchKailhChoc(switch_type=switch_type, path3d=path3d))
        ...
        for key in keys:
            switches.append(
                SwitchKailhChoc(
                    ...
                    path3d=path3d,
                    keycap=Keycap(x_spacing=x_spacing, y_spacing=y_spacing, **keycaps[key]),
                )
            )
...
def generate_switch_kailh_choc_mini(output_path):
    ...
    for key in keys:
        switches.append(
            SwitchKailhChocMini(path3d=path3d, keycap=Keycap(x_spacing=x_spacing, y_spacing=y_spacing, **keycaps[key]))
        )
...
def generate_switch_kailh_kh(output_path):
    ...
    for key in keys:
        switches.append(SwitchKailhKH(path3d=path3d, keycap=Keycap(spacing=spacing, **keycaps[key])))
...
def generate_switch_kailh_nb(output_path):
    ...
    for key in keys:
        switches.append(SwitchKailhNB(path3d=path3d, keycap=Keycap(spacing=spacing, **keycaps[key])))
...
def generate_switch_hotswap_kailh(output_path):
    ...
    for plated in [False, True]:
        ...
        for key in keys:
            switches.append(
                SwitchHotswapKailh(path3d=path3d, hotswap_plated=plated, keycap=Keycap(spacing=spacing, **keycaps[key]))
            )
...
def generate_switch_hotswap_kailh_choc(output_path):
    ...
    for plated in [False, True]:
        switches.append(SwitchKailhChoc(path3d=path3d, hotswap=True, hotswap_plated=plated))
